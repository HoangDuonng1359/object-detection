...
Không được sử dụng các bộ phát hiện đối tượng hoàn chỉnh như YOLOv5/v8, Detectron2, MMDetection, hoặc Faster R-CNN/SSD có sẵn trong torchvision. Được phép dùng PyTorch, các lớp mạng cơ bản, và mạng trích xuất đặc trưng đã huấn luyện trước như resnet

## Bộ Dữ Liệu

[Link download dữ liệu](https://drive.google.com/file/d/1itXnaiYd0Z8sj4KjpV-x9esZOuykjkd8/view?usp=share_link)
...
1. Kiểm tra JSON có đúng định dạng không.
2. Kiểm tra hộp bao hợp lệ và nằm trong ảnh.
3. Kiểm tra nhãn lớp hợp lệ.
4. Tính IoU, độ chính xác (precision), độ bao phủ (recall) và mAP@0.5 trên tập kiểm tra ẩn.
